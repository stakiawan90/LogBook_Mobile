...
export default function Pending() {
  ...
  const titleColor = isDarkTheme ? "#f8fafc" : "#d97706";
  ...
  useEffect(() => {
    async function loadPendingLogs() {
      setLoading(true);
      try {
        const fetchedLogs = await getAdminLogsByStatus("pending");
        setLogs(fetchedLogs);
      } catch (error: any) {
        Alert.alert("Unable to load pending logs", error?.message || "Please check your connection.");
      } finally {
        setLoading(false);
      }
    }
  ...
  return (
    <RNSSafeAreaView style={[styles.container, { backgroundColor: containerBg }]}> 
      <Text style={[styles.title, { color: titleColor }]}>Pending Logbook</Text>
