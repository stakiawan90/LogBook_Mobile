...
export default function Completed() {
  ...
  useEffect(() => {
    async function loadCompletedLogs() {
      setLoading(true);
      try {
        const fetchedLogs = await getAdminLogsByStatus("completed");
        setLogs(fetchedLogs);
      } catch (error: any) {
        Alert.alert("Unable to load completed logs", error?.message || "Please check your connection.");
      } finally {
        setLoading(false);
      }
    }
  ...
  return (
    <RNSSafeAreaView style={[styles.container, { backgroundColor: containerBg }]}> 
      <Text style={[styles.title, { color: titleColor }]}>Completed Logbook</Text>
          ...
          {filteredLogs.map((item) => (
            <View key={item.id} style={[styles.card, { backgroundColor: cardBg, borderColor }]}> 
              ...
              <View style={styles.timeRow}>
                ...
                <Text style={styles.time}>Time Out: {item.time_out || "-"}</Text>
              </View>
